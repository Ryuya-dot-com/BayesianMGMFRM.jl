...
implemented, together with a four-cell sequential scaling plan. Short-NUTS
execution remains explicit and is blocked before generation when its
free-memory gate is not met. Process-lifetime maxRSS is recorded but is not
labelled probe-attributable peak memory. Execution in a suitable environment,
sequential scaled review, and isolated-process peak-RSS review precede grid/
resource freezing and independent threshold review. The attempt-complete
