...
implemented. Short-NUTS execution remains explicit and is blocked before
generation when its free-memory gate is not met. Its short-chain values cannot
establish convergence, peak memory, or full NUTS runtime. Execution in a
suitable environment and scaled resource review precede grid/resource freezing
and independent threshold review. The attempt-complete executor and evaluation
with untouched seeds follow.
