...
The Active Decision Roadmap below governs new work. Later progress ledgers and
LD1b checklists are retained as historical snapshots, not as the current work
order or package-completion scores.
...
| A. Freeze the validation protocol | **P0, in progress.** `mgmfrm_validation_protocol()` now records identified-block and heldout-response estimands; the source-literal `1.7` versus reference `1.702` scale policy; primary between-item Q, boundary within/mixed Q, and Q-misspecification scope; connected sparse design; actual prior refits; a paired CmdStan subset; layered structural, computational, scientific, and robustness criteria; and five-category response-pattern stress. `ordinal_response_pattern_audit()` supplies the per-dataset audit; `mgmfrm_response_stress_plan()`, `simulate_mgmfrm_response_stress()`, and `mgmfrm_response_stress_preflight()` implement the nine-attempt MCMC-free generation/preflight layer with a preserved denominator and typed failures. No embedded scientific threshold is inferred from pilots. Fit and diagnostic attempt phases and independent threshold review remain blockers. | A reviewable, portable protocol and commands exist; the fit runner retains every attempt and stress stratum, and independently reviewed scientific thresholds are frozen before any fresh evaluation starts. |
...
Passing this preflight is generation and structural evidence only; the
attempt-complete fit and diagnostic phases remain open.
...
but the older stress runner is not wired to them, so the full gate stays
closed. The next gate is independent threshold review plus an attempt-complete
runner; pilots remain limited to operability/runtime and must not freeze
scientific thresholds from their outcomes. Evaluation then uses untouched
seeds.
