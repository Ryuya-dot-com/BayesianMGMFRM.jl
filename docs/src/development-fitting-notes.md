# Bayesian Fitting Development Notes
...
`thresholds = :partial_credit`, `discrimination = :rater`, has no anchors or
fitted DFF terms, and remains on the specified-only manifest path; it returns
[`MGMFRMFit`](@ref) for a fixed-Q confirmatory `family = :mgmfrm` candidate
with `dimensions >= 2`, `thresholds = :partial_credit`, generic
`discrimination = :none`, and no anchors or fitted DFF terms. The MGMFRM
kernel still estimates Q-masked item-dimension discrimination; the generic
`:none` selector prevents an unrelated discrimination option from being
silently ignored. Unsupported generalized options, public
...
The scalar GMFRM experimental candidate remains guarded. Its source-aligned Julia
...
Reproducing the committed completed-batch evidence requires
`generate_mgmfrm_publication_grade_refit_batch_results_review.jl
--read-local-artifacts`. Without that explicit flag, the generator safely treats
local runner outputs as absent. The full-paper archive records this command and
stages downstream generators according to their non-null fixture-SHA
dependencies, with its documented existence-only cycle breaks handled
separately.
...
`family = :mgmfrm`, `dimensions >= 2`, `thresholds = :partial_credit`, generic
`discrimination = :none`, empty anchors and DFF terms, and a fixed
item-by-dimension `q_matrix`; then `fit(spec; experimental = true)` returns an
[`MGMFRMFit`](@ref). The
...
facets_report(fit_result; by = :rater)
...
parameter, the resolved source-aligned raw-prior scale, and the raw-density
...
simulations or fit models. A deterministic validation-plan export records those
rows' controls, coverage summary, and falsification-rule contract
...
with `target = :all`. `fit_stats` returns posterior summaries of infit and
outfit mean-square statistics by facet level and remains the default Bayesian
residual diagnostic. The separate [`facets_report`](@ref) helper (also
available as [`facets_compatibility_stats`](@ref))
uses the parameter-wise posterior mean as a plugin point estimate for a
fit-supported MFRM/RSM/PCM fit. For unit-weighted observations in a facet
group, it computes

```math
\mathrm{Infit}=\frac{\sum_i r_i^2}{\sum_i V_i},\qquad
\mathrm{Outfit}=\frac{1}{N}\sum_i\frac{r_i^2}{V_i},
```

then applies the Wright--Masters fourth-moment convention

```math
df_{\mathrm{Infit}}=
\frac{2(\sum_i V_i)^2}{\sum_i(C_i-V_i^2)},\qquad
df_{\mathrm{Outfit}}=
\frac{2N^2}{\sum_i(C_i/V_i^2-1)},
```

where ``r_i`` is observed minus plugin-expected score, ``V_i`` is the
conditional score variance, and ``C_i`` is the conditional fourth central
moment. ZSTD uses

```math
Z=\frac{\mathrm{MnSq}^{1/3}-(1-2/(9df))}{\sqrt{2/(9df)}}
```

and is capped at ``\pm9`` by default. Every row labels the result as a
FACETS-compatible Bayesian plugin approximation: it does not propagate
posterior uncertainty and does not claim numerical equivalence to FACETS JML
output. Non-unit weights and `GMFRMFit`/`MGMFRMFit` inputs are rejected rather
than silently extending the formula to generalized discrimination models.
`fair_average_summary` returns
