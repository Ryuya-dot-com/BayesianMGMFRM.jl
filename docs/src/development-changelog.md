# BayesianMGMFRM.jl Development Change Ledger
...
- Make guarded GMFRM/MGMFRM numerical execution fail closed: a shared
  ...
  evaluation paths before they can silently reuse a different generalized
  kernel. MGMFRM heldout refit scoring retains strict structural Q checks while
  allowing the one-row scoring slice itself to omit observations from another
  valid dimension; model construction and training still require full Q/data
  coverage validation.
- Make the full reproduction manifest dependency-ordered and self-checking:
  include both publication-grade execution reviews, preserve completed local
  batch evidence with `--read-local-artifacts`, reject incomplete 5-by-25
  model-weight surfaces, and test every non-null fixture-SHA edge against the
  recorded command order. The archive now also evaluates the guarded exposure
  dependency through `all_local_evidence_passed` before the weaker `reviewed`
  marker, so a reviewed but failed evidence packet cannot pass aggregation.
- Add `facets_report` (with `facets_compatibility_stats` as a descriptive
  alias) as an explicit MFRM/RSM/PCM-only
  posterior-mean plugin table with unit-weighted infit/outfit MnSq,
  Wright--Masters fourth-moment df, Wilson--Hilferty ZSTD capped at +/-9, and
  machine-readable approximation caveats. Existing posterior `fit_stats`
  defaults are unchanged; non-unit weights and generalized fits are rejected.
...
- Relabel the completed 30-45 day roadmap section as a sprint record so the
  current remaining boundary is the independent scope-review/registration gate rather
...
- Add repository-only registration handoff guidance for release maintainers.
- Tighten the release-verification gate to check the General AutoMerge-facing
  package-name rules and origin URL shape before handoff.
- Split the CI hygiene mode from the full public-language scan,
...
- Add a local registration handoff script that verifies the manual release
  ...
  registration action.
- Add a Zotero-informed, DOI-traced literature-anchored synthetic benchmark
  generator and committed artifact with the smallest Uto and Ueno (2020)
  scalar-GMFRM cell and a fixed-Q adaptation of the smallest two-dimensional
  Uto (2021) cell using a loading-weighted ability sum. The artifact stores
  `1,700` deterministic synthetic ratings, parameter truth, separate
  truth/response seeds, hashes, exact-versus-adapted provenance, and
  source-equation oracle checks while leaving recovery, TAM/FACETS/ConQuest,
  construct-validity, and independent-review claims blocked.
- Add a deterministic TAM overlap-baseline generator with a long-format CSV for
  `TAM::tam.mml.mfr`, known-truth MFRM partial-credit parameters,
  package-oracle probability checks, and explicit pre-execution status.
- Add a local TAM execution-review generator and committed artifact that runs
  `TAM::tam.mml.mfr`, extracts the parameter table, records diagnostic
  item/rater/item-step comparisons, and keeps external-validation and public
  release claims blocked pending predeclared thresholds and adapter review.
- Audit the TAM formula adapter through the expanded constraint table and
  category-intercept reconstruction, and add a prospective comparison-policy
  artifact. The item/rater pilot clears the future numerical gates, while the
  item-step pilot misses its correlation and mean-absolute-difference gates;
  multi-replication comparison and public external-validation claims remain
  blocked.
- Execute the frozen TAM policy over 30 known-truth replications spanning 40,
  100, and 250 persons. All three parameter blocks pass in 10/10 primary
  250-person replications; item-step rises from 6/10 at 40 persons to 10/10 at
  100 and 250 persons. The artifact remains TAM-recovery evidence rather than a
  direct package-posterior agreement or broad external-validation claim.
- Add a direct same-data package-versus-TAM estimate pilot. A four-chain,
  1,600-draw AdvancedHMC fit has zero divergences, maximum classical split
  R-hat about 1.002, and minimum ESS about 999; centered item, centered rater,
  and reconstructed item-step posterior means correlate above 0.99 with TAM.
  Because agreement thresholds were not frozen before this pilot, the result
  remains descriptive and public external-validation claims stay blocked.
- Freeze a prospective direct package-versus-TAM agreement policy after the
  descriptive pilot and before any direct multi-replication package fits. The
  policy records item/rater/item-step correlation, absolute-difference,
  interval-coverage, and sampler-diagnostic gates for future runs while keeping
  external-validation and public claims blocked.
- Add a multiaxial refinement sidecar for that frozen policy without changing
  any primary threshold or fit count. It separates direct numerical agreement
  from known-truth recovery, records that the descriptive item-step pilot shows
  close package/TAM agreement without full truth-recovery support, fixes
  seed/data/fit/retry/failure contracts, makes 4-of-5 and block-level
  discreteness explicit, and limits any future passing result to the fully
  crossed unit-discrimination MFRM/PCM overlap slice.
- Execute the unchanged direct package-versus-TAM policy on five fresh
  40-person and five fresh 100-person datasets. All ten package fits pass the
  frozen sampler gate, all ten TAM fits pass their numerical/adapter audit, and
  item, rater, and item-step direct agreement pass 5/5 in the primary
  100-person condition; both package and TAM truth-recovery qualifiers also
  pass 5/5 for every primary block. Four rank-normalized R-hat advisories remain
  visible without changing the prospectively frozen classical diagnostic gate.
...
- Thread the TAM result, all-attempt audit, and post-execution packet through
  ...
  exposure reviews as MFRM-only non-transfer evidence. The upper reviews keep
  the two TAM review blockers explicit without disabling existing guarded local
  scalar GMFRM or fixed-Q MGMFRM entrypoints.
- Fix the local JSON writer's `AbstractDict` key iteration so JSON3 objects from
  retained job results can be serialized into aggregate artifacts.
- Add a literature-anchored independent-review packet generator and committed
  artifact that freezes the synthetic benchmark, generator source, hashes,
  exact/adapted labels, loading-weighted ability-sum note, and claim ledger for
  reviewer handoff while keeping signed review and public claim release blocked.
...
- Add a local MGMFRM Q-revision construct-validity review generator and
  ...
  candidates non-public, and clears the construct-review blocker without
...
- Add a local MGMFRM scalar-remediation comparison artifact that compares the
  ...
  batch expansion, records a restricted scalar batch sampler policy, and keeps
...
- Add a local MGMFRM fit-threshold/Q/heldout linkage generator and JSON
  ...
  validation.
- Expose release-verification gate availability in `release_scope_summary` and
...
- Add a local prediction-target/model-weight policy generator and committed JSON
  ...
  sparse-superiority claims blocked at that gate until independent scope
  review.
...
- Add `examples/guarded_mgmfrm.jl`, a compact fixed-Q confirmatory MGMFRM
  guarded-fit example, and run it from the release-verification gate.
...
- Add a deterministic validation-plan
...
- Expose internal GMFRM/MGMFRM raw-parameterization manifests on preview
  ...
  fitting blocked.
- Add a private scalar GMFRM validation-candidate path that records candidate
...
- Split the scalar GMFRM experimental candidate from the source-fixture blueprint
...
- Extend that GMFRM experimental-candidate path with constrained direct-parameter
  metadata and raw-to-direct transform diagnostics that verify source
  constraints and pointwise log-likelihood agreement.
- Add a private GMFRM validation-candidate direct pointwise fixture API that
...
- Add a private GMFRM validation-candidate sampler diagnostic surface that
...
- Add a local scalar GMFRM guarded-exposure review generator and committed JSON
  ...
  sparse-design and WAIC influence fixtures, records the review as non-public,
...
- Add `GMFRMFit` and a guarded scalar GMFRM
  ...
  rater-discrimination experimental candidate, plus a guarded fit
...
- Extend the GitHub Actions test matrix to Windows in addition to Ubuntu and
  macOS.
- Add a release-verification script and CI hygiene-gate job for
