# BayesianMGMFRM.jl Development README Ledger
...
`thresholds = :partial_credit` and the compatibility keyword
`discrimination = :rater`, and fixed-Q confirmatory MGMFRM with
`dimensions >= 2`, `thresholds = :partial_credit`, and generic
`discrimination = :none` are available only through guarded experimental
paths. Both require empty anchors and no fitted DFF terms. Broader generalized
discrimination, rating-scale generalized kernels, exploratory MGMFRM, free
latent correlations, modeled DFF effects, sparse-superiority claims, and
public model-weight claims remain out of scope.
...
To test the current development branch instead, install it directly from
GitHub:
...
- `posterior_summary`, `fair_average_summary`,
  ...
  report-ready measurement and pathway-map rows. The separate
  `facets_report` helper (also available as
  `facets_compatibility_stats`) returns explicitly approximate, unit-weighted
  posterior-mean plugin rows with Wright--Masters fourth-moment df and capped
  Wilson--Hilferty ZSTD for minimal MFRM/RSM/PCM fits only.
...
- `fit_artifact`, `fit_report`, `save_fit_report`, `save_fit_report_tables`,
  ...
  `fit_report_dossier` create hash-checked analysis artifacts and portable
  review summaries.
...
| Scalar rater-consistency GMFRM | `experimental` | `mfrm_spec(...; family = :gmfrm, thresholds = :partial_credit, discrimination = :rater, anchors = [])`, then `fit(spec; experimental = true)` | Narrow guarded source-aligned candidate; DFF terms must be empty and `discrimination = :rater` selects the positive rater-consistency surface. |
| Fixed-Q confirmatory MGMFRM | `experimental` | `mfrm_spec(...; family = :mgmfrm, dimensions = D, thresholds = :partial_credit, discrimination = :none, q_matrix = Q, anchors = [])`, then `fit(spec; experimental = true)` | Guarded fixed Q-mask candidate for `D >= 2`; DFF terms must be empty and identity latent correlation is fixed. The kernel still estimates Q-masked item-dimension discrimination. |
...
- Read [`docs/src/scope.md`](docs/src/scope.md) for the conservative scope
  boundary and release direction.
