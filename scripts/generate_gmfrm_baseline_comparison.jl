...
const DEFAULT_OUTPUT = joinpath(ROOT, "test", "fixtures", "gmfrm_baseline_comparison.json")

include(joinpath(@__DIR__, "local_json.jl"))

const RECOVERY_PROTOCOL = GMFRMRecoverySmoke.PROTOCOL

const PROTOCOL = (;
    protocol_id = "scalar_gmfrm_baseline_comparison_v1",
    simulation_source = :scalar_gmfrm_recovery_smoke_v1,
    ...
    baseline_sampler = (;
        ...
        split_chains = true,
        seeds = (;
            partial_credit = 20260742,
            rating_scale = 20260743,
        ),
    ),
    diagnostics = (;
        rhat_threshold = 1.2,
...
function usage()
    return """
    Generate the local scalar GMFRM baseline-comparison artifact.

    Usage:
      julia --project=. scripts/generate_gmfrm_baseline_comparison.jl [--output PATH]
    """
end
...
function simulated_gmfrm_table()
    placeholder_design = GMFRMRecoverySmoke.scalar_gmfrm_design(
        GMFRMRecoverySmoke.placeholder_table(),
    )
    direct_truth =
        BayesianMGMFRM._gmfrm_source_constrained_params_from_unconstrained(
            placeholder_design,
            GMFRMRecoverySmoke.TRUTH_RAW,
        )
    table = GMFRMRecoverySmoke.simulated_table(placeholder_design, direct_truth)
...
function gmfrm_candidate_record(table)
    ...
    diagnostics = BayesianMGMFRM._gmfrm_promotion_candidate_sampler_diagnostics(
        target,
        zeros(length(GMFRMRecoverySmoke.TRUTH_RAW));
        seed = sampler.seed,
...
function baseline_comparison_artifact()
    simulated = simulated_gmfrm_table()
    table = simulated.table
    data = GMFRMRecoverySmoke.facet_data(table)
    gmfrm = gmfrm_candidate_record(table)
    ...
    model_results = [gmfrm, partial_credit, rating_scale]
    ...
    return (;
        schema = "bayesianmgmfrm.gmfrm_baseline_comparison.v1",
        family = :gmfrm,
        ...
        simulated_data = (;
            ...
            truth_source = :gmfrm_recovery_smoke_truth,
            raw_parameter_order_sha256 =
                gmfrm.record.parameter_order_sha256,
            direct_parameter_order_sha256 =
                gmfrm.record.direct_parameter_order_sha256,
        ),
        model_rows = rows,
        summary = (;
            passed,
            ...
            recommendation = :keep_internal_until_guarded_exposure_decision,
        ),
    )
end
...
function main(args)
    ...
    println("passed=", artifact.summary.passed,
        ...
        " gmfrm_rank=", artifact.summary.gmfrm_rank,
        " gmfrm_elpd_diff=", artifact.summary.gmfrm_elpd_difference)
    return nothing
end
