...
const DEFAULT_OUTPUT =
    joinpath(ROOT, "test", "fixtures",
        "mgmfrm_full_heldout_mcmc_refit_candidate_batch_scoring.json")
...
const PROTOCOL = (;
    protocol_id = "mgmfrm_full_heldout_mcmc_refit_candidate_batch_scoring_v1",
    review_kind = :local_full_heldout_mcmc_refit_candidate_batch_scoring,
    publication_or_registration_action = false,
    local_only = true,
    pilot_only = true,
    smoke_only = true,
    execution_scope = :all_fold_fixed_q_mgmfrm_candidate_scoring,
    ...
    comparison_anchor_policy = :recorded_not_refit_or_scored_in_candidate_batch,
    scoring_target = :heldout_pointwise_log_predictive_density,
    expected_score_target = :observed_minus_expected_score_residual,
    fit_seed_policy = :execution_plan_seed_plus_candidate_batch_offset,
    fit_controls = (;
        backend = :advancedhmc,
        sampler = :nuts,
        chains = 1,
    ...
    thresholds = (;
        ...
        require_all_candidate_folds_selected = true,
        require_all_candidate_refits_succeeded = true,
        require_all_candidate_scores_recorded = true,
        require_all_pointwise_scores_recorded = true,
        require_all_score_values_finite = true,
        require_all_folds_covered_for_each_candidate = true,
        require_scenario_model_kfold_rows_recorded = true,
        require_candidate_rank_rows_recorded = true,
        require_comparison_anchors_recorded_not_scored = true,
        require_full_125_unit_batch_not_completed = true,
        require_publication_grade_diagnostics_blocked = true,
        require_anchor_model_comparison_blocked = true,
...
function usage()
    return """
    Generate the local MGMFRM all-fold fixed-Q candidate heldout scoring artifact.

    This extends the fold-1 scoring pilot to every deterministic heldout fold
    for the three fixed-Q MGMFRM candidate models. Scalar and null/reference
    comparison anchors are recorded as not refit or scored in this candidate
    batch, so full 125-unit execution, model weights, sparse-superiority,
    Q-revision, and public fit claims remain blocked.

    Usage:
      julia --project=. scripts/generate_mgmfrm_full_heldout_mcmc_refit_candidate_batch_scoring.jl [--output PATH]
    """
end
...
function model_batch_family(model::Symbol)
    model in MGMFRM_CANDIDATE_MODELS && return :mgmfrm_fixed_q_candidate
...
function candidate_batch_unit_rows(plan)
    units = NamedTuple[]
    for row in collect(plan[:execution_unit_rows])
        model = as_symbol(row[:model])
        family = model_batch_family(model)
        fit_attempted = family === :mgmfrm_fixed_q_candidate
        profile =
            fit_attempted ? FoldPilotHelpers.q_profile(as_symbol(row[:scenario]), model) :
            nothing
        push!(units, (;
            ...
            batch_family = family,
            batch_role = fit_attempted ?
                :fixed_q_mgmfrm_candidate_refit_scored :
                :comparison_anchor_recorded_not_scored,
            fit_attempted,
            q_profile = fit_attempted ? profile.q_profile : missing,
            q_matrix =
                fit_attempted ? FoldPilotHelpers.q_matrix_rows(profile.q_matrix) :
                missing,
            ...
            batch_chains = fit_attempted ? PROTOCOL.fit_controls.chains : 0,
            batch_draws_per_chain =
                fit_attempted ? PROTOCOL.fit_controls.draws : 0,
            batch_warmup_per_chain =
                fit_attempted ? PROTOCOL.fit_controls.warmup : 0,
            plan_random_seed = as_int(row[:random_seed]),
...
candidate_units(units) = [unit for unit in units if Bool(unit.fit_attempted)]

function fit_and_score_candidate(unit)
    rows = FoldPilotHelpers.synthetic_rows(unit)
    train_rows = [row for row in rows if row.split_role === :train]
    q_matrix = FoldPilotHelpers.q_matrix_from_rows(unit.q_matrix)
    ...
    fit_seed = Int(unit.plan_random_seed) + PROTOCOL.fit_controls.seed_offset
    ...
    score_row = (;
        ...
        n_q_validation_warnings = Int(q_validation.summary.n_warning_rows),
...
function score_rows_and_pointwise(units)
    ...
    pointwise_rows = NamedTuple[]
    for unit in candidate_units(units)
        result = fit_and_score_candidate(unit)
...
function fold_completion_rows(score_rows, pointwise_rows)
    rows = NamedTuple[]
    ...
    for scenario in scenarios, fold in folds
        ...
        push!(rows, (;
            scenario,
            fold,
            n_candidate_models_scored = length(fold_scores),
            n_heldout_pointwise_rows = length(fold_pointwise),
            heldout_observations =
                sort(unique(row.observation for row in fold_pointwise)),
            candidate_fold_complete =
                length(fold_scores) == length(MGMFRM_CANDIDATE_MODELS) &&
        ...
        push!(rows, (;
            ...
            publication_grade_diagnostics_blocked =
                all(row -> Bool(row.publication_grade_diagnostics_blocked),
                    model_scores),
            comparison_scope =
                :fixed_q_mgmfrm_candidates_all_folds_only,
            public_model_weight_claim_allowed = false,
            sparse_superiority_claim_allowed = false,
        ))
    end
...
function candidate_rank_rows(kfold_rows)
    rows = NamedTuple[]
    scenarios = sort(unique(row.scenario for row in kfold_rows); by = string)
    for scenario in scenarios
        scenario_rows =
            sort([row for row in kfold_rows if row.scenario === scenario];
        ...
        for (rank, row) in enumerate(scenario_rows)
            push!(rows, (;
                ...
                best_model_in_candidate_batch = rank == 1,
                comparison_scope =
                    :fixed_q_mgmfrm_candidates_all_folds_only,
                public_model_weight_claim_allowed = false,
                sparse_superiority_claim_allowed = false,
...
function anchor_rows(units)
    return [
        (execution_unit_id = unit.execution_unit_id,
            scenario = unit.scenario,
            model = unit.model,
            fold = unit.fold,
            batch_family = unit.batch_family,
            batch_role = unit.batch_role,
            fit_attempted = false,
            heldout_predictive_score_computed = false,
            not_scored_reason =
                :comparison_anchor_not_refit_in_fixed_q_candidate_batch,
            required_followup =
                :run_anchor_refits_or_full_125_unit_batch_before_model_comparison,
            public_claim_allowed = false)
        for unit in units
...
function build_artifact()
    ...
    units = candidate_batch_unit_rows(plan)
    candidate_batch_units = candidate_units(units)
    scored = score_rows_and_pointwise(units)
    score_rows = scored.score_rows
    pointwise_rows = scored.pointwise_rows
    fold_completion = fold_completion_rows(score_rows, pointwise_rows)
    kfold_rows = scenario_model_kfold_rows(score_rows, pointwise_rows)
    ranks = candidate_rank_rows(kfold_rows)
    anchors = anchor_rows(units)
    ...
    fold1_scoring_passed = Bool(fold1_record.summary.passed)
    ...
    expected_candidate_units =
        Int(heldout_execution_record.summary.n_scenarios) *
        length(MGMFRM_CANDIDATE_MODELS) *
        Int(heldout_execution_record.summary.n_folds)
    all_candidate_folds_selected =
        length(candidate_batch_units) == expected_candidate_units &&
        all(unit -> Int(unit.fold) in PROTOCOL.selected_folds,
            candidate_batch_units)
    all_candidate_refits_succeeded =
        length(score_rows) == expected_candidate_units &&
        all(row -> Bool(row.fit_succeeded) && Bool(row.layout_matches) &&
                Bool(row.q_validation_passed),
            score_rows)
    all_candidate_scores_recorded =
        length(score_rows) == expected_candidate_units
    expected_pointwise_rows =
        sum(unit.n_heldout_observations for unit in candidate_batch_units)
    ...
    all_folds_covered_for_each_candidate =
        all(row -> Bool(row.all_folds_scored), kfold_rows) &&
        all(row -> Bool(row.candidate_fold_complete), fold_completion)
    scenario_model_kfold_rows_recorded =
        length(kfold_rows) ==
        Int(heldout_execution_record.summary.n_scenarios) *
        length(MGMFRM_CANDIDATE_MODELS)
    candidate_rank_rows_recorded =
        length(ranks) == length(kfold_rows) &&
        all(scenario -> count(row -> row.scenario === scenario, ranks) ==
                        length(MGMFRM_CANDIDATE_MODELS),
            unique(row.scenario for row in kfold_rows))
    ...
    publication_grade_diagnostics_blocked =
        all(row -> Bool(row.publication_grade_diagnostics_blocked),
            score_rows)
    anchor_model_comparison_blocked =
        comparison_anchors_recorded_not_scored &&
        !Bool(fold1_record.summary.comparison_anchor_scores_computed)
    ...
    passed = all_input_artifacts_present &&
        ...
        heldout_prediction_execution_passed &&
        validation_split_policy_passed &&
        Bool(fold1_record.summary.fold1_heldout_predictive_scores_computed) &&
        all_candidate_folds_selected &&
        all_candidate_refits_succeeded &&
        all_candidate_scores_recorded &&
        all_pointwise_scores_recorded &&
    ...
    n_review_cells =
        length(candidate_batch_units) + length(score_rows) +
        length(pointwise_rows) + length(fold_completion) +
        length(kfold_rows) + length(ranks) + length(anchors)
    total_heldout_elpd =
        sum(row.heldout_elpd for row in score_rows; init = 0.0)
    total_heldout_observations =
    ...
    return (;
        ...
        family = :mgmfrm,
        scope = :full_heldout_mcmc_refit_candidate_batch_scoring,
        status = :fixed_q_mgmfrm_candidate_all_fold_scores_recorded,
        decision =
        ...
        candidate_execution_unit_rows = candidate_batch_units,
        candidate_score_rows = score_rows,
        heldout_pointwise_rows = pointwise_rows,
        fold_completion_rows = fold_completion,
        scenario_model_kfold_rows = kfold_rows,
        ...
        summary = (;
            ...
            n_candidate_execution_unit_rows = length(candidate_batch_units),
            n_candidate_score_rows = length(score_rows),
            n_heldout_pointwise_rows = length(pointwise_rows),
            n_fold_completion_rows = length(fold_completion),
            n_scenario_model_kfold_rows = length(kfold_rows),
            ...
            n_models = length(MGMFRM_CANDIDATE_MODELS),
            n_folds = length(PROTOCOL.selected_folds),
            n_candidate_heldout_observations = total_heldout_observations,
            total_heldout_elpd,
            mean_heldout_log_predictive_density =
                total_heldout_elpd / total_heldout_observations,
            ...
            n_full_execution_units_completed = length(score_rows),
