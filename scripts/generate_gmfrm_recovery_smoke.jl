...
const DEFAULT_OUTPUT = joinpath(ROOT, "test", "fixtures", "gmfrm_recovery_smoke.json")
...
const TRUTH_RAW = [
    ...
    -0.25, 0.1,
    log(1.25), log(0.9),
    log(1.1), log(0.9), log(1.0),
    0.2, -0.15, 0.05,
]

const PROTOCOL = (;
    protocol_id = "scalar_gmfrm_recovery_smoke_v1",
    simulation_seed = 20260701,
    sampler_seed = 20260702,
    grid = (;
        persons = 4,
        items = 3,
        raters = 3,
        ...
        item_discrimination_variation = :moderate,
    ...
    thresholds = (;
        ...
        max_block_mean_absolute_error = 0.5,
        max_parameter_absolute_error = 1.0,
...
function usage()
    return """
    Generate the local scalar GMFRM recovery-smoke artifact.

    Usage:
      julia --project=. scripts/generate_gmfrm_recovery_smoke.jl [--output PATH]
    """
end
...
function scalar_gmfrm_design(table)
...
function sample_scores(design, direct_params; rng::AbstractRNG)
    rows = BayesianMGMFRM._gmfrm_source_fixture_values(design, direct_params)
    scores = Int[]
...
function recovery_artifact()
    placeholder_design = scalar_gmfrm_design(placeholder_table())
    placeholder_target = BayesianMGMFRM._gmfrm_promotion_candidate_logdensity(
        placeholder_design,
    )
    direct_truth = BayesianMGMFRM._gmfrm_source_constrained_params_from_unconstrained(
        placeholder_design,
        TRUTH_RAW,
    )
    table = simulated_table(placeholder_design, direct_truth)
    design = scalar_gmfrm_design(table)
    ...
    diagnostics = BayesianMGMFRM._gmfrm_promotion_candidate_sampler_diagnostics(
        ...
        target_accept = PROTOCOL.sampler.target_accept,
        max_depth = PROTOCOL.sampler.max_depth,
        max_energy_error = PROTOCOL.sampler.max_energy_error,
        metric = PROTOCOL.sampler.metric,
        ad_backend = PROTOCOL.sampler.ad_backend,
    ...
    return (;
        schema = "bayesianmgmfrm.gmfrm_recovery_smoke.v1",
        family = :gmfrm,
