...
const DEFAULT_OUTPUT_MD =
    joinpath(ROOT, "artifacts", "uto_style_draws_x4_gate_followup",
        "uto_style_draws_x4_gate_followup.md")

include(joinpath(@__DIR__, "local_json.jl"))

const OUTPUT_SCHEMA =
    "bayesianmgmfrm.mgmfrm_uto_style_draws_x4_gate_followup.v1"

function usage()
    return """
    Execute the draws_x4 gate after a successful draws_x2 smoke follow-up.

    This reruns the same priority model/split cells with the plan's
    `draws_x4_gate` profile and compares the result against both the original
    warning surface and the draws_x2 smoke run. It is local diagnostic evidence
    only.

    Usage:
      julia --project=. scripts/generate_mgmfrm_uto_style_draws_x4_gate_followup.jl [options]
    ...
    Options:
      ...
      --max-jobs N                 Limit gate jobs. Default: all draws-x2 jobs.
...
function parse_args(args)
    ...
    return (; plan_json, warning_surface_json, draws_x2_json, output_json,
        output_md, max_jobs, progress)
end
...
function gate_jobs(draws_x2, max_jobs::Int)
    rows = [(;
        sequence = int_value(row.sequence),
        source_job = symbol_value(row.job),
        job = Symbol(replace(string(row.job), "draws_x2_smoke" => "draws_x4_gate")),
        ...
        success_check = :clear_or_explain_remaining_rhat_ess_warnings,
...
function draws_x2_model_row(draws_x2, job)
    matches = [row for row in draws_x2.model_diagnostic_rows
    ...
    isempty(matches) && error("draws-x2 row not found for $(job.job)")
    return only(matches)
end
...
function delta(new_value, old_value)
    return round4(finite_float(new_value) - finite_float(old_value))
end

function comparison_row(job, baseline, draws_x2, draws_x4)
    return (;
        job = job.job,
        ...
        draws_x4_flag = draws_x4.diagnostic_flag,
        baseline_max_rhat = round_or_missing(baseline.max_rhat),
        draws_x2_max_rhat = round_or_missing(draws_x2.max_rhat),
        draws_x4_max_rhat = draws_x4.max_rhat,
        delta_x4_max_rhat_vs_baseline =
            delta(draws_x4.max_rhat, baseline.max_rhat),
        delta_x4_max_rhat_vs_x2 = delta(draws_x4.max_rhat, draws_x2.max_rhat),
        baseline_min_ess = round_or_missing(baseline.min_ess),
        draws_x2_min_ess = round_or_missing(draws_x2.min_ess),
        draws_x4_min_ess = draws_x4.min_ess,
        delta_x4_min_ess_vs_baseline =
            delta(draws_x4.min_ess, baseline.min_ess),
        delta_x4_min_ess_vs_x2 = delta(draws_x4.min_ess, draws_x2.min_ess),
        baseline_bad_rhat = int_value(baseline.n_bad_rhat),
        draws_x2_bad_rhat = int_value(draws_x2.n_bad_rhat),
        draws_x4_bad_rhat = draws_x4.n_bad_rhat,
        delta_x4_bad_rhat_vs_x2 =
            int_value(draws_x4.n_bad_rhat) - int_value(draws_x2.n_bad_rhat),
        baseline_low_ess = int_value(baseline.n_low_ess),
        draws_x2_low_ess = int_value(draws_x2.n_low_ess),
        draws_x4_low_ess = draws_x4.n_low_ess,
        delta_x4_low_ess_vs_x2 =
            int_value(draws_x4.n_low_ess) - int_value(draws_x2.n_low_ess),
        cleared_warning = draws_x4.diagnostic_flag === :ok,
        improved_vs_x2_min_ess =
            finite_float(draws_x4.min_ess) > finite_float(draws_x2.min_ess),
        improved_vs_x2_max_rhat =
            finite_float(draws_x4.max_rhat) < finite_float(draws_x2.max_rhat),
        reduced_vs_x2_warning_counts =
            int_value(draws_x4.n_bad_rhat) <= int_value(draws_x2.n_bad_rhat) &&
            int_value(draws_x4.n_low_ess) <= int_value(draws_x2.n_low_ess),
        public_claim_allowed = false,
    )
...
function block_delta_rows(job, surface, draws_x2, draws_x4_block_rows)
    rows = NamedTuple[]
    for block in job.target_blocks
        base = block_row(surface.block_diagnostic_rows, job, block)
        x2 = block_row(draws_x2.block_diagnostic_rows, job, block)
        x4 = block_row(draws_x4_block_rows, job, block)
        (base === nothing || x2 === nothing || x4 === nothing) && continue
        push!(rows, (;
            job = job.job,
            model = job.model,
            split_offset = job.split_offset,
            ...
            draws_x2_max_rhat = round_or_missing(x2.max_rhat),
            draws_x4_max_rhat = round_or_missing(x4.max_rhat),
            delta_x4_max_rhat_vs_x2 = delta(x4.max_rhat, x2.max_rhat),
            baseline_min_ess = round_or_missing(base.min_ess),
            draws_x2_min_ess = round_or_missing(x2.min_ess),
            draws_x4_min_ess = round_or_missing(x4.min_ess),
            delta_x4_min_ess_vs_x2 = delta(x4.min_ess, x2.min_ess),
            delta_x4_bad_rhat_vs_x2 =
                int_value(x4.n_bad_rhat) - int_value(x2.n_bad_rhat),
            delta_x4_low_ess_vs_x2 =
                int_value(x4.n_low_ess) - int_value(x2.n_low_ess),
            public_claim_allowed = false,
        ))
    end
    return rows
...
function execute_job(plan, surface, draws_x2, profile, job, progress::Bool)
    result = DrawsX2.execute_job(surface, profile, job, progress)
    baseline = DrawsX2.baseline_model_row(surface, job)
    x2 = draws_x2_model_row(draws_x2, job)
    comp = comparison_row(job, baseline, x2, result.model_row)
    blocks = block_delta_rows(job, surface, draws_x2, result.block_rows)
...
function finding_rows(comparisons)
    ...
    n_cleared = count(row -> Bool(row.cleared_warning), comparisons)
    n_min_ess = count(row -> Bool(row.improved_vs_x2_min_ess), comparisons)
    n_rhat = count(row -> Bool(row.improved_vs_x2_max_rhat), comparisons)
    n_counts =
        count(row -> Bool(row.reduced_vs_x2_warning_counts), comparisons)
    return [
        (finding = :draws_x4_gate_executed,
            severity = :info,
            evidence = string(n, " priority model/split job(s)"),
            implication = :second_budget_followup_completed,
            public_claim_allowed = false),
        (finding = :warning_clearance,
            severity = n_cleared == n ? :info : :warning,
            evidence = string(n_cleared, "/", n,
                " jobs cleared mcmc warnings"),
            implication = :do_not_expand_claims_until_warnings_clear_or_are_explained,
            public_claim_allowed = false),
        (finding = :incremental_min_ess_response,
            severity = n_min_ess == n ? :info : :warning,
            evidence = string(n_min_ess, "/", n,
                " jobs improved min ESS beyond draws_x2"),
            implication = :assess_whether_draws_x4_is_enough,
            public_claim_allowed = false),
        (finding = :incremental_rhat_response,
            severity = n_rhat == n ? :info : :warning,
            evidence = string(n_rhat, "/", n,
                " jobs improved max R-hat beyond draws_x2"),
            implication = :consider_chain_count_or_parameterization_if_rhat_stalls,
            public_claim_allowed = false),
        (finding = :public_claims_blocked,
            severity = :blocker,
            evidence = "local draws_x4 gate only",
            implication =
...
function render_markdown(path, artifact)
    mkpath(dirname(path))
    open(path, "w") do io
        println(io, "# Uto-Style Draws-x4 Gate Follow-Up")
        println(io)
        ...
        table(io, ["Job", "Flag", "dESS vs x2", "dRhat vs x2",
                "dBad vs x2", "dLow vs x2"],
            [[row.job, row.draws_x4_flag, row.delta_x4_min_ess_vs_x2,
                row.delta_x4_max_rhat_vs_x2, row.delta_x4_bad_rhat_vs_x2,
                row.delta_x4_low_ess_vs_x2]
             for row in artifact.comparison_rows])
        println(io, "## Target Block Deltas")
        table(io, ["Job", "Block", "dESS vs x2", "dRhat vs x2",
                "dBad vs x2", "dLow vs x2"],
            [[row.job, row.block, row.delta_x4_min_ess_vs_x2,
                row.delta_x4_max_rhat_vs_x2, row.delta_x4_bad_rhat_vs_x2,
                row.delta_x4_low_ess_vs_x2]
        ...
        println(io)
        println(io,
            "This gate tests whether the priority warning surface clears under ",
            "the larger retained-draw profile. It is not public threshold or ",
...
function build_artifact(options)
    ...
    jobs = gate_jobs(draws_x2, options.max_jobs)
    ...
    for job in jobs
        result = execute_job(plan, surface, draws_x2, profile, job,
            options.progress)
    ...
    return (;
        ...
        family = :mgmfrm,
        scope = :uto_style_draws_x4_gate_followup,
        status = :local_draws_x4_gate_recorded,
        ...
        summary = (;
            ...
            n_min_ess_improved_vs_x2 =
                count(row -> Bool(row.improved_vs_x2_min_ess), comparisons),
            n_max_rhat_improved_vs_x2 =
                count(row -> Bool(row.improved_vs_x2_max_rhat), comparisons),
            n_warning_counts_reduced_vs_x2 =
                count(row -> Bool(row.reduced_vs_x2_warning_counts),
                    comparisons),
            ...
            next_gate = all(row -> Bool(row.cleared_warning), comparisons) ?
                :expand_draws_x4_to_all_split_stable_models :
                :run_chain_count_gate_and_parameterization_audit,
        ),
    )
end
...
function main(args = ARGS)
    ...
    println("jobs=", artifact.summary.n_jobs,
        ...
        " min_ess_improved_vs_x2=",
        artifact.summary.n_min_ess_improved_vs_x2,
        " rhat_improved_vs_x2=",
        artifact.summary.n_max_rhat_improved_vs_x2,
