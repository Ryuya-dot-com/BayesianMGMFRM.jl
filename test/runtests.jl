...
function check_gmfrm_full_paper_reproduction_archive_fixture(
    ...
    @test Bool(thresholds[
        :require_mgmfrm_q_revision_construct_validity_review_passed])
    @test Bool(thresholds[
    ...
    expected_paths = Dict(
        ...
        "mgmfrm_q_matrix_validation_expansion" =>
    ...
    @test any(row -> String(row[:path]) ==
        ...
        code_doc_records)
    @test any(row -> String(row[:path]) ==
        "scripts/generate_mgmfrm_heldout_prediction_validation_policy.jl",
        code_doc_records)
    @test any(row -> String(row[:path]) ==
    ...
    @test any(row -> String(row[:artifact]) ==
        "mgmfrm_q_revision_construct_validity_review", full_commands)
    @test any(row -> String(row[:artifact]) ==
...
function check_gmfrm_manuscript_scale_simulation_grid_fixture(
    ...
    @test any(row -> String(row[:gate]) ==
        "mgmfrm_q_revision_construct_validity_review" &&
        Int(row[:n_evidence_cells]) == 2 &&
        String(row[:key_check]) ==
            "mgmfrm_q_revision_construct_validity_review", evidence_rows)
    @test any(row -> String(row[:gate]) ==
        "mgmfrm_guarded_local_fit_entrypoint" &&
        Int(row[:n_evidence_cells]) == 2 &&
        String(row[:key_check]) ==
            "mgmfrm_guarded_local_fit_entrypoint", evidence_rows)
    @test any(row -> String(row[:gate]) ==
        "mgmfrm_fit_metric_threshold_sensitivity" &&
    ...
    @test any(row -> String(row[:gate]) ==
        "mgmfrm_heldout_prediction_validation_policy" &&
        Int(row[:n_evidence_cells]) == 14 &&
        String(row[:key_check]) ==
            "mgmfrm_heldout_prediction_validation_policy", evidence_rows)
    @test any(row -> String(row[:gate]) ==
        "mgmfrm_validation_split_model_comparison_policy" &&
...
function check_gmfrm_guarded_exposure_review_fixture(fixture_path::AbstractString)
    ...
    @test any(row -> String(row[:gate]) ==
        ...
        Bool(row[:evidence]), review_rows)
    @test any(row -> String(row[:gate]) ==
        "confirmatory_mgmfrm_heldout_prediction_validation_policy" &&
        String(row[:status]) == "passed_with_policy_blocker" &&
        Bool(row[:evidence]), review_rows)
    @test any(row -> String(row[:gate]) ==
...
function check_mgmfrm_q_matrix_validation_expansion_fixture(
    ...
    @test String(fixture[:family]) == "mgmfrm"
    @test String(fixture[:scope]) == "minimal_confirmatory_mgmfrm_candidate"
    @test String(fixture[:status]) == "q_matrix_validation_expansion_recorded"
    @test String(fixture[:decision]) == "keep_guarded_fixed_q_only"
    @test Bool(fixture[:public_fit])
    @test Bool(fixture[:experimental_public])
    @test Bool(fixture[:fit_ready])
    ...
    thresholds = protocol[:thresholds]
    @test String(protocol[:protocol_id]) ==
        "confirmatory_mgmfrm_q_matrix_validation_expansion_v1"
    @test String(protocol[:review_kind]) ==
...
function check_mgmfrm_q_candidate_real_fit_diagnostic_linkage_fixture(
    ...
    @test String(fixture[:family]) == "mgmfrm"
    @test String(fixture[:scope]) == "q_candidate_real_fit_diagnostic_linkage"
    @test String(fixture[:status]) ==
        "q_candidate_real_fit_diagnostic_linkage_recorded"
    @test String(fixture[:decision]) ==
        "keep_candidate_q_fit_linkage_diagnostic_only"
    @test Bool(fixture[:public_fit])
    @test Bool(fixture[:experimental_public])
    @test Bool(fixture[:fit_ready])
    @test Bool(fixture[:empirical_q_recovery_public]) == false
...
function check_mgmfrm_q_revision_construct_validity_review_fixture(
    ...
    @test String(fixture[:status]) ==
        "q_revision_construct_validity_review_recorded"
    @test String(fixture[:decision]) ==
...
function check_mgmfrm_guarded_local_fit_entrypoint_fixture(
    ...
    @test String(fixture[:family]) == "mgmfrm"
    @test String(fixture[:scope]) ==
        "construct_reviewed_q_guarded_local_fit_entrypoint"
    @test String(fixture[:status]) ==
        "guarded_local_fit_entrypoint_recorded"
    @test String(fixture[:decision]) ==
...
function check_mgmfrm_fit_metric_threshold_sensitivity_fixture(
    ...
    @test String(fixture[:family]) == "mgmfrm"
    @test String(fixture[:scope]) ==
        "construct_reviewed_q_fit_metric_threshold_sensitivity"
    @test String(fixture[:status]) ==
        "fit_metric_threshold_sensitivity_recorded"
    @test String(fixture[:decision]) ==
...
function check_mgmfrm_heldout_prediction_validation_policy_fixture(
    ...
    @test String(fixture[:family]) == "mgmfrm"
    @test String(fixture[:scope]) == "heldout_prediction_validation_policy"
    @test String(fixture[:status]) ==
        "heldout_prediction_validation_policy_recorded"
    @test String(fixture[:decision]) ==
...
function check_mgmfrm_validation_split_model_comparison_policy_fixture(
    ...
    @test String(fixture[:family]) == "mgmfrm"
    @test String(fixture[:scope]) ==
        "validation_split_model_comparison_policy"
    @test String(fixture[:status]) ==
        "validation_split_model_comparison_policy_recorded"
    @test String(fixture[:decision]) ==
...
@testset "minimal MFRM spec and design" begin
    ...
    mgmfrm_q_matrix_validation_expansion_fixture = optional_fixture_path("MFRM_MGMFRM_Q_MATRIX_VALIDATION_EXPANSION_FIXTURE", joinpath("test", "fixtures", "mgmfrm_q_matrix_validation_expansion.json"))
    if !isempty(mgmfrm_q_matrix_validation_expansion_fixture)
        check_mgmfrm_q_matrix_validation_expansion_fixture(
            mgmfrm_q_matrix_validation_expansion_fixture,
        )
    end
    mgmfrm_empirical_q_matrix_recovery_policy_fixture = optional_fixture_path("MFRM_MGMFRM_EMPIRICAL_Q_MATRIX_RECOVERY_POLICY_FIXTURE", joinpath("test", "fixtures", "mgmfrm_empirical_q_matrix_recovery_policy.json"))
    ...
    mgmfrm_q_revision_construct_validity_review_fixture = optional_fixture_path("MFRM_MGMFRM_Q_REVISION_CONSTRUCT_VALIDITY_REVIEW_FIXTURE", joinpath("test", "fixtures", "mgmfrm_q_revision_construct_validity_review.json"))
    if !isempty(mgmfrm_q_revision_construct_validity_review_fixture)
        check_mgmfrm_q_revision_construct_validity_review_fixture(
            mgmfrm_q_revision_construct_validity_review_fixture,
        )
    end
    mgmfrm_guarded_local_fit_entrypoint_fixture = optional_fixture_path("MFRM_MGMFRM_GUARDED_LOCAL_FIT_ENTRYPOINT_FIXTURE", joinpath("test", "fixtures", "mgmfrm_guarded_local_fit_entrypoint.json"))
    if !isempty(mgmfrm_guarded_local_fit_entrypoint_fixture)
        check_mgmfrm_guarded_local_fit_entrypoint_fixture(
            mgmfrm_guarded_local_fit_entrypoint_fixture,
        )
    end
    mgmfrm_fit_metric_threshold_sensitivity_fixture = optional_fixture_path("MFRM_MGMFRM_FIT_METRIC_THRESHOLD_SENSITIVITY_FIXTURE", joinpath("test", "fixtures", "mgmfrm_fit_metric_threshold_sensitivity.json"))
    ...
    mgmfrm_heldout_prediction_validation_policy_fixture = optional_fixture_path("MFRM_MGMFRM_HELDOUT_PREDICTION_VALIDATION_POLICY_FIXTURE", joinpath("test", "fixtures", "mgmfrm_heldout_prediction_validation_policy.json"))
    if !isempty(mgmfrm_heldout_prediction_validation_policy_fixture)
        check_mgmfrm_heldout_prediction_validation_policy_fixture(
            mgmfrm_heldout_prediction_validation_policy_fixture,
        )
    end
    mgmfrm_validation_split_model_comparison_policy_fixture = optional_fixture_path("MFRM_MGMFRM_VALIDATION_SPLIT_MODEL_COMPARISON_POLICY_FIXTURE", joinpath("test", "fixtures", "mgmfrm_validation_split_model_comparison_policy.json"))
