...
function check_fixed_q_report_bundle(input, directory; view = :full)
    canonical = input isa B.MultidimensionalMFRMFit
    result = canonical ? B._canonical_mfrm_fixed_q_samples(input) : input
    save_bundle = canonical ? save_fit_report_bundle : B._save_mfrm_fixed_q_report_bundle
    path = joinpath(directory, "fixed-q-figures")
    extension = Base.get_extension(B, :BayesianMGMFRMCairoMakieExt)
    if extension === nothing
        @test_throws ArgumentError save_bundle(path, input; figures = (posterior = (;),))
        @test !ispath(path)
        return
    end
    ...
    bounds = canonical ? (; posterior_lower = 0.1, posterior_upper = 0.9, view) : (; posterior_interval = 0.8)
    options = (; bounds..., predictive_interval = 0.8, draw_indices = indices, seed = 42)
    before = deepcopy(result.record)
    Random.seed!(918); next_random = rand(); Random.seed!(918)
    manifest = save_bundle(path, input; figures, options...,
    ...
    @test report["family"] == (canonical ? "mfrm" : "mfrm_fixed_q")
    @test report["report_status"] == "complete"
    @test report["metadata"]["source_sample_content_hash"] == result.record.content_hash
    @test report["metadata"]["prior"]["scales"] == B._json_export_value(result.record.prior.scales)
    @test report["metadata"]["diagnostic_settings"] == B._json_export_value(B._mfrm_fixed_q_metadata(result).diagnostic_settings)
    @test report["dimension_labels"] == result.record.spec.dimension_labels
    if canonical
        @test report["metadata"]["estimation_status"] == "experimental"
        @test B.plot_posterior(input; block = :person, dimension = result.record.spec.dimension_labels[2]) isa extension.Figure
        @test B.plot_diagnostics(input; parameters = [person, fixed]) isa extension.Figure
        @test B.plot_predictive(input; interval = 0.8, draw_indices = indices, seed = 42) isa extension.Figure
        @test_throws ArgumentError B.plot_posterior(input; scale = :raw)
        @test_throws ArgumentError B.plot_diagnostics(input; dimension = "absent")
        @test_throws ArgumentError B.plot_predictive(input; ndraws = 0)
    end
    ...
    original = files(path)
    @test_throws ArgumentError save_bundle(path, input; overwrite = true,
    ...
    for extra in ((; figures = (wright = (;),)), (; seed = true),
        ...
        @test_throws ArgumentError save_bundle(path, input; overwrite = true, extra...)
        @test files(path) == original
    end
    if canonical
        for extra in ((; posterior_lower = 0.2), (; view = :unknown),
                (; rhat_threshold = result.record.run.checked.rhat_threshold + 0.01))
            @test_throws ArgumentError save_bundle(path, input; overwrite = true, figures, extra...)
            @test files(path) == original
        end
    end
    ...
    restored = canonical ? B._mfrm_fixed_q_fit(loaded) : loaded
    save_bundle(replay, restored; figures = (predictive = (;),), options...)
