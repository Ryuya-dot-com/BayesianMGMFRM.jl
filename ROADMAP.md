...
sensitivity, retry, resource, and threshold decisions, so the full robustness
gate stays closed. The next gate is to freeze those decisions after runtime-
only resource work and independent threshold review, then implement the
attempt-complete executor. Evaluation then uses untouched seeds.
