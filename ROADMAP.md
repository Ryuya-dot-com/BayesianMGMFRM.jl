...
diagonal-run replay. A subsequent dense-metric attempt timed out during `fit`
without a saved result; its mixing remains unevaluated. Make collective location
diagnostics visible and localize the remaining computational cost before another
long fit. Scientific margins and independent
...
| --- | --- | --- |
| 1. Expose collective location diagnostics and localize remaining cost — analyst/maintainer | The [dense-metric C2 attempt](docs/internal/fixed-coefficient-validation-protocol.md#dense-metric-julia-comparison-2026-09-17) reached its 1,800 s deadline during `fit`, without a saved result; mixing and efficiency remain unevaluated. The preserved diagonal run fitted in 548 s, and both original backends show slow person/item means. Next expose finite-panel means and invariant contrasts through existing diagnostics/report/plot routes, including saved Julia/CmdStan results. Before another dense fit, distinguish compilation, warmup, retained sampling and diagnostic construction in a bounded cost probe | Exit: retain the same joint prior, likelihood, coordinates/Jacobian and diagnostic criteria. Label finite-panel means correctly and preserve original diagnostic records. A timeout does not establish statistical inferiority; neither timing nor same-seed replay establishes scientific acceptance. Keep all attempts; do not extend the failed run, hard-center the fitted model, select a default from one panel, or start the grid. MFRM-wide validation is not a prerequisite for the next relevant MGMFRM slice |
