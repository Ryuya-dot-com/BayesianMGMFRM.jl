...
| --- | --- | --- |
| 1. Act on the C2 computation pilot — analyst/maintainer | Remove demonstrated repeated structural work from the Julia density/gradient path using existing validated model structures; measure cold fit, persistence and report costs separately. Then investigate absolute item-location mixing against the already declared focal contrasts | Exit: retain input/mutation guards and the same prior, likelihood, coordinates and Jacobian; verify densities/gradients against existing references and CmdStan before a separately bounded follow-up. Preserve the original timeout and compiler failure. Do not loosen diagnostics after seeing results, replace primary attempts, start the grid or add another controller. MFRM-wide validation is not a prerequisite for the next relevant MGMFRM slice |
