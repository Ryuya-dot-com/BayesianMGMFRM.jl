...
exposes repeated design/Q validation inside the likelihood path. Investigate
that measured cost while preserving the target and boundary checks, then address
location mixing separately from focal contrasts. Scientific margins and independent
...
| --- | --- | --- |
| 1. Address collective location mixing — analyst/maintainer | The [phase-separated C2 follow-up](docs/internal/fixed-coefficient-validation-protocol.md#phase-separated-julia-follow-up-2026-09-17) completed fitting in 548 s and reproduced all 4,000 Julia draws exactly; save/reload/report timings and the post-reload recipe failure/recovery are recorded. Both backends show slow person/item means despite well-mixed location-invariant contrasts. Next compare the existing dense metric in a separately declared bounded attempt, and expose finite-panel location summaries through diagnostic UX | Exit: retain the same joint prior, likelihood, coordinates/Jacobian and diagnostic criteria. Timing and same-seed replay do not establish convergence or scientific acceptance. Preserve every attempt, including worse outcomes; do not replace the original failures, hard-center the fitted model, choose a default from one panel, or start the grid. MFRM-wide validation is not a prerequisite for the next relevant MGMFRM slice |
