...
| --- | --- | --- |
| 1. Follow up the C2 computation correction — analyst/maintainer | Blueprint reuse now removes repeated design compilation from the shared MGMFRM gradient path; the [local warm-gradient probe](docs/internal/fixed-coefficient-validation-protocol.md#julia-blueprint-reuse-after-the-c2-pilot-2026-09-17) ran 2.45–2.82 times as fast and allocated 93.0% fewer bytes. Next measure cold fit, save, reload and report phases in a separately bounded follow-up, then investigate absolute item-location mixing against the declared focal contrasts | Exit: retain input/mutation guards and the same prior, likelihood, coordinates and Jacobian. Warm-gradient timing does not close whole-fit runtime or statistical acceptance. Preserve the original timeout and compiler failure; do not loosen diagnostics, replace primary attempts, start the grid or add another controller. MFRM-wide validation is not a prerequisite for the next relevant MGMFRM slice |
