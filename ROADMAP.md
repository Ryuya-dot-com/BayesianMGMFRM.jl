...
| --- | --- | --- |
| 1. Expose collective location diagnostics and localize remaining cost — analyst/maintainer | The [authorized 60-minute C2 rerun](docs/internal/fixed-coefficient-validation-protocol.md#user-authorized-deadline-extension-2026-09-18) completed fit/save/reload/report/qualification in 57.22 minutes and met the pilot and additional location thresholds; the original 30-minute timeout is preserved. Dense fitting took 3,214 s versus diagonal 548 s; person-mean minimum bulk ESS rose from 357 to 5,634, while already well-mixed contrasts gained little. Next expose finite-panel means and invariant contrasts through existing diagnostics/report/plot routes, including saved Julia/CmdStan results. Follow the deeper dense warmup in a bounded cost probe before another long fit | Exit: retain the same joint prior, likelihood, coordinates/Jacobian and diagnostic criteria. Label finite-panel means correctly and preserve original diagnostic records. One panel's diagnostic gains do not establish calibration, a universal speed advantage or a new default. Keep all attempts and their declared deadlines; do not hard-center the fitted model or start the grid. MFRM-wide validation is not a prerequisite for the next relevant MGMFRM slice |
